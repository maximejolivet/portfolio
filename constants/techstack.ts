import type { TechCategory } from '~/types/content.types'

export const TECH_CATEGORIES: TechCategory[] = [
  {
    id: 'languages',
    labelKey: 'techSection.categories.languages.label',
    descriptionKey: 'techSection.categories.languages.description',
    items: [
      { id: 'php', name: 'PHP', icon: 'material-icon-theme:php' },
      { id: 'javascript', name: 'JavaScript', icon: 'logos:javascript' },
    ],
  },
  {
    id: 'frontend',
    labelKey: 'techSection.categories.frontend.label',
    descriptionKey: 'techSection.categories.frontend.description',
    items: [
      { id: 'html', name: 'HTML', icon: 'logos:html-5' },
      { id: 'modern-css', name: 'Modern CSS', icon: 'devicon:css' },
      { id: 'vue', name: 'Vue.js', icon: 'logos:vue' },
      { id: 'nuxt', name: 'Nuxt', icon: 'logos:nuxt-icon' },
      { id: 'tailwind', name: 'Tailwind', icon: 'logos:tailwindcss-icon' },
      { id: 'bootstrap', name: 'Bootstrap', icon: 'logos:bootstrap' },
    ],
  },
  {
    id: 'backend',
    labelKey: 'techSection.categories.backend.label',
    descriptionKey: 'techSection.categories.backend.description',
    items: [
      { id: 'symfony', name: 'Symfony', icon: 'logos:symfony' },
      { id: 'laravel', name: 'Laravel', icon: 'logos:laravel' },
    ],
  },
  {
    id: 'cms',
    labelKey: 'techSection.categories.cms.label',
    descriptionKey: 'techSection.categories.cms.description',
    items: [
      { id: 'drupal', name: 'Drupal', icon: 'logos:drupal-icon' },
      { id: 'wordpress', name: 'WordPress', icon: 'logos:wordpress-icon' },
      { id: 'webflow', name: 'Webflow', icon: 'skill-icons:webflow' },
    ],
  },
  {
    id: 'database',
    labelKey: 'techSection.categories.database.label',
    descriptionKey: 'techSection.categories.database.description',
    items: [
      { id: 'mysql', name: 'MySQL', icon: 'logos:mysql' },
      { id: 'postgresql', name: 'PostgreSQL', icon: 'logos:postgresql' },
      { id: 'redis', name: 'Redis', icon: 'logos:redis' },
    ],
  },
  {
    id: 'ai',
    labelKey: 'techSection.categories.ai.label',
    descriptionKey: 'techSection.categories.ai.description',
    items: [
      { id: 'genai', name: 'IA générative', icon: 'lucide:sparkles' },
      { id: 'llm', name: 'LLM', icon: 'lucide:brain-circuit' },
      { id: 'prompt-engineering', name: 'Prompt Engineering', icon: 'lucide:terminal' },
      { id: 'rag', name: 'RAG', icon: 'lucide:database-zap' },
      { id: 'vector-db', name: 'Bases vectorielles', icon: 'lucide:vector-square' },
      { id: 'agents', name: 'Agents', icon: 'lucide:bot' },
      { id: 'skills', name: 'Skills', icon: 'lucide:zap' },
      { id: 'mcp', name: 'MCP', icon: 'lucide:plug-zap' },
    ],
  },
  {
    id: 'aiTools',
    labelKey: 'techSection.categories.aiTools.label',
    descriptionKey: 'techSection.categories.aiTools.description',
    items: [
      { id: 'github-copilot', name: 'GitHub Copilot', icon: 'logos:github-copilot' },
      { id: 'claude-code', name: 'Claude Code', icon: 'logos:claude-icon' },
      { id: 'cursor', name: 'Cursor', icon: 'devicon-plain:cursor' },
      { id: 'openai-codex', name: 'OpenAI Codex', icon: 'logos:openai-icon' },
      { id: 'ollama', name: 'Ollama', icon: 'selfhst:ollama' },
      { id: 'chatgpt', name: 'ChatGPT', icon: 'logos:openai-icon' },
      { id: 'claude-ai', name: 'Claude.ai', icon: 'logos:claude-icon' },
    ],
  },
  {
    id: 'devops',
    labelKey: 'techSection.categories.devops.label',
    descriptionKey: 'techSection.categories.devops.description',
    items: [
      { id: 'docker', name: 'Docker', icon: 'logos:docker-icon' },
      { id: 'cicd', name: 'Pipelines CI/CD', icon: 'lucide:workflow' },
      { id: 'vercel', name: 'Vercel', icon: 'logos:vercel-icon' },
      { id: 'gcp', name: 'Google Cloud', icon: 'logos:google-cloud' },
      { id: 'firebase', name: 'Firebase', icon: 'selfhst:firebase' },
      { id: 'supabase', name: 'Supabase', icon: 'logos:supabase-icon' },
    ],
  },
  {
    id: 'security',
    labelKey: 'techSection.categories.security.label',
    descriptionKey: 'techSection.categories.security.description',
    items: [
      { id: 'ssl', name: 'SSL/TLS', icon: 'lucide:lock' },
      { id: 'csp', name: 'CSP', icon: 'lucide:shield' },
      { id: '2fa', name: '2FA', icon: 'lucide:shield-check' },
      { id: 'snyk', name: 'Snyk', icon: 'logos:snyk' },
      { id: 'eslint', name: 'ESLint', icon: 'logos:eslint' },
      { id: 'prettier', name: 'Prettier', icon: 'logos:prettier' },
      { id: 'phpstan', name: 'PHPStan', icon: 'material-icon-theme:phpstan' },
      { id: 'rgaa', name: 'Accessibilité RGAA', icon: 'lucide:accessibility' },
    ],
  },
  {
    id: 'testing',
    labelKey: 'techSection.categories.testing.label',
    descriptionKey: 'techSection.categories.testing.description',
    items: [
      { id: 'postman', name: 'Postman', icon: 'logos:postman-icon' },
      { id: 'bruno', name: 'Bruno', icon: 'devicon:bruno' },
      { id: 'phpunit', name: 'PHPUnit', icon: 'material-icon-theme:phpunit' },
      { id: 'functional-tests', name: 'Tests fonctionnels', icon: 'lucide:clipboard-check' },
      { id: 'uat', name: 'Recette fonctionnelle', icon: 'lucide:clipboard-list' },
    ],
  },
  {
    id: 'tools',
    labelKey: 'techSection.categories.tools.label',
    descriptionKey: 'techSection.categories.tools.description',
    items: [
      { id: 'phpstorm', name: 'PhpStorm', icon: 'devicon:phpstorm' },
      { id: 'vscode', name: 'Visual Studio Code', icon: 'logos:visual-studio-code' },
      { id: 'git', name: 'Git', icon: 'logos:git-icon' },
      { id: 'github', name: 'GitHub', icon: 'logos:github-icon' },
      { id: 'gitlab', name: 'GitLab', icon: 'material-icon-theme:gitlab' },
      { id: 'canva', name: 'Canva', icon: 'devicon:canva' },
      { id: 'figma', name: 'Figma', icon: 'logos:figma' },
      { id: 'notion', name: 'Notion', icon: 'logos:notion-icon' },
      { id: 'jira', name: 'Jira', icon: 'logos:jira' },
      { id: 'mantis', name: 'Mantis', icon: 'lucide:bug' },
      { id: 'matomo', name: 'Matomo', icon: 'selfhst:matomo' },
      { id: 'google-analytics', name: 'Google Analytics', icon: 'logos:google-analytics' },
    ],
  },
  {
    id: 'watch',
    labelKey: 'techSection.categories.watch.label',
    descriptionKey: 'techSection.categories.watch.description',
    items: [
      { id: 'python', name: 'Python', icon: 'logos:python' },
      { id: 'django', name: 'Django', icon: 'logos:django-icon' },
      { id: 'react', name: 'React', icon: 'logos:react' },
      { id: 'typescript', name: 'TypeScript', icon: 'logos:typescript-icon' },
    ],
  },
]
