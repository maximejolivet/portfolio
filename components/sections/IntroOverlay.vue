...
<template>
  <div v-if="visible"
    ...
    :class="fading ? 'opacity-0' : 'opacity-100'" @click="finish">
    <div class="absolute left-8 top-7 flex items-center gap-2.5 transition-opacity duration-500 ease-out"
      :class="detailsVisible ? 'opacity-100' : 'opacity-0'">
      <UiLogoMark :size="30" class="text-panel-foreground" />
      <span class="font-mono text-[0.75rem] font-medium text-panel-foreground/60">www.maxime.bzh</span>
    </div>
