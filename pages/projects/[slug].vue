...
const liveIndex = computed(() =>
  categoryLiveCaseStudies.value.findIndex((p) => p.slug === slug.value),
)
const hasAdjacentProjects = computed(
  () => project.value.live && categoryLiveCaseStudies.value.length > 1,
)
...
<template>
  <div>
    <LayoutPageSection bare>
      <UiContainer class="max-w-[1080px] pt-8">
        <NuxtLink
      ...
      <UiContainer v-if="project.live" class="max-w-[1280px] pt-8">
        <div class="relative">
          ...
          <div v-if="project.image" class="overflow-hidden rounded-[28px] border border-border">
            <NuxtImg
              :src="project.image"
      ...
      <UiContainer class="max-w-[1080px]">
        <header class="flex flex-col gap-5 pb-12 pt-10">
          ...
          <p
            class="max-w-[600px] text-pretty font-sans text-base leading-[1.7] text-muted-foreground"
          >
        ...
        <template v-if="project.live">
          <section class="grid items-start gap-16 pb-18 lg:grid-cols-[minmax(0,1fr)_300px]">
                ...
                <div class="mt-1.5 flex flex-col gap-2.5">
                  ...
                  >
                    <UiAppIcon
                      ...
                      class="mt-1.5 size-3.5 shrink-0 text-mint"
                    />
            ...
            >
              ...
              <div class="flex items-baseline gap-2.5">
                <span
                  class="size-2 shrink-0 -translate-y-px animate-pulse-dot rounded-full bg-mint"
                />
          ...
          <nav
            v-if="hasAdjacentProjects"
            class="flex justify-between gap-6 border-t border-border py-10"
          >
            <NuxtLink
            ...
            >
              ...
              <span
                class="font-sans text-[1.125rem] font-bold tracking-[-0.3px] text-foreground hover:text-accent"
              >
        ...
        <div
          v-else
          class="flex flex-col items-center gap-4 border-t border-border py-24 text-center"
        >
          <UiBadge>{{ $t('projectsPage.underConstruction') }}</UiBadge>
          <p
            class="max-w-[420px] text-pretty font-sans text-sm leading-[1.7] text-muted-foreground"
          >
